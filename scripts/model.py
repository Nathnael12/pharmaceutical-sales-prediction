...
logger =Logger("model.log").get_app_logger()
...
class Model:
    ...
    def preprocess(self):
        preprocess_pipeline = Pipeline(steps=[
            ('prepare', FunctionTransformer(processor.prepare, validate=False)),
            ('target_feature_split', FunctionTransformer(self.split_target_feature, kw_args={'target_col':'Sales'}, validate=False)),
            ('train_test_split', FunctionTransformer(util.train_test_split,kw_args={'size':(.8,.2)}, validate=False))
        ])
