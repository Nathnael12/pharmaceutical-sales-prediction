...
def insert_to_table(dbName: str, df: pd.DataFrame, table_name: str) -> None:
    ...
    for _, row in df.iterrows():
        # length.append(row)
        sqlQuery = f"""INSERT INTO {table_name} (Store, DayOfWeek, Date,
        Sales, Customers, Open, Promo, StateHoliday, SchoolHoliday,
        StoreType, Assortment, CompetitionDistance, CompetitionOpenSinceMonth,
        CompetitionOpenSinceYear, Promo2, Promo2SinceWeek, PromoInterval, Weekday,
        Weekend, Month, Year, Day, PeriodInMonth, Before_holiday, After_holiday) 
        VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s);"""
        data = (row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7], row[8], row[9], row[10], row[11], row[12], row[13], row[14], row[15], row[16],row[17],row[18],row[19],row[20],row[21],row[22],row[23],row[24])
...
if __name__ == "__main__":
    ...
    latest_df=utils.read_from_dvc("data/featured_train.csv",repo,"featured-v0",low_memory=False)
